...
export function Skills({ variant = 'full', containWidth }: SkillsProps) {
  if (variant === 'rail') {
    return (
      <div className="space-y-5" role="region" aria-labelledby="skills-rail-title">
        <div>
          <p className="text-[10px] font-bold uppercase tracking-[0.22em] text-accent">
            Capabilities
          </p>
          <h2 id="skills-rail-title" className="mt-2 text-xl font-semibold tracking-tight text-foreground">
            Skills
          </h2>
        ...
        <div className="space-y-3">
          {skillGroups.map((group, i) => (
            <Reveal key={group.title} delay={i * 0.04}>
              <div className="rounded-xl border border-border/80 bg-card/50 p-3.5 shadow-card backdrop-blur-sm transition-shadow hover:border-accent/20">
                <h3 className="text-xs font-semibold tracking-tight text-foreground">{group.title}</h3>
                <ul className="mt-2.5 flex flex-wrap gap-1.5">
                  {group.items.map((item) => (
                    <li
                      key={item}
                      className="rounded-md border border-border/60 bg-background/45 px-2 py-0.5 text-[10px] font-medium leading-tight text-foreground/90"
                    >
                      {item}
                    </li>
                  ))}
